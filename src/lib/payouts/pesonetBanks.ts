 ...
 *
 * THIS IS NOT RAIL-AGNOSTIC, AND THE UPLOAD SCREEN DEFAULTS TO THE OTHER
 * RAIL. PayMongo's bulk-transfer page has a "Send via" choice between
 * InstaPay and PESONet, and — per a screenshot of the live screen (August
 * 2026) — InstaPay is the one selected by default. The uploaded file is
 * interpreted against whichever is chosen. Every name below is a PESONet
 * spelling, so a file built from these values must be uploaded with PESONet
 * selected; uploading it under InstaPay risks rejection at best. Anything
 * that generates a transfer file from this list should say so plainly where
 * the person doing the uploading will actually see it.
 *
