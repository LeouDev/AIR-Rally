...
# PayMongo Platforms — Marketplace 95%/5% Split (in progress)

Extends the seam above with PayMongo's real Platforms/FIaaS product: venues become PayMongo child ("merchant") accounts, and a booking's PayMongo checkout session can carry a `split_payment` that routes 5% to AIR/Rally and 95% to the venue automatically. The plain, non-split PayMongo flow documented above keeps working unmodified for any venue that isn't onboarded — this is an *extension* of the existing checkout call, not a parallel path.

## Research discipline: real API calls and raw docs, not paraphrase

Before writing any code, every mechanism below was confirmed two ways: (1) real, live `curl` calls against this TEST MODE account (`POST /v2/accounts`, `.../identity_verification`, `.../activate`, `POST /v2/checkout_sessions` with `split_payment`), and (2) raw markdown fetched directly from `docs.paymongo.com` (bypassing an AI-summarization layer once, after it produced a paraphrase — `hosted_url` — that a live response actually returned as `url`). Two real findings shaped the design:

- **`Workflows`** (PayMongo's separate automation engine for scheduled/triggered money movement) is explicitly gated: *"Account configuration is required before testing transactions. If you are interested in access to this feature, please contact support@paymongo.com!"* — not used here. The `split_payment` field on Checkout Sessions is a **different, already-available** mechanism, confirmed directly from PayMongo's own raw OpenAPI schema and a real Linked Transactions doc example.
- **A real TEST MODE inconsistency**: `POST /v2/accounts/{id}/activate`'s own response claims `activation_status: "activated"`, but a `GET` on the same account — even seconds later — still showed `"pending"`. Because of this, activation status is tracked **only** via the `merchant.activated`/`merchant.declined` webhook (PayMongo's own documented "reliable signal"), never inferred from the synchronous `activate` call.

## Database (Phase 1 — 3 migrations, additive, live)

- `20260810000009_paymongo_marketplace.sql`: `venues.paymongo_account_id`/`paymongo_activation_status` (`unlinked`/`pending`/`under_review`/`activated`/`declined` — `unlinked` is AIR/Rally's own sentinel, not PayMongo's) + onboarding timestamps; `bookings.platform_fee_amount`/`venue_amount`/`paymongo_venue_account_id` (nullable — set only for a split PayMongo booking).
- `20260810000010_paymongo_venue_status_fix.sql`: a real design bug caught before any code called it — the original single `sync_venue_paymongo_status()` required a `venue_id`, but the `merchant.activated` webhook only ever carries the PayMongo account id, with no metadata pass-through on Accounts to embed one. Split into `sync_venue_paymongo_status()` (owner-initiated, once, at onboarding start) and `sync_venue_paymongo_activation()` (webhook-only, looked up purely by the already-linked, unique `paymongo_account_id`).
- `20260810000011_paymongo_split_snapshot_fix.sql`: a second real bug — `platform_fee_amount`/`venue_amount`/`paymongo_venue_account_id` were guarded as strictly bypass-only (`price_amount`'s class), but they're actually meant to be set by the booking's own owner at checkout-session-creation time, the same self-service moment as `paymongo_checkout_session_id`. Reclassified into that category instead. Safe either way: these three columns are an audit snapshot never read back to decide a live checkout session's real split (PayMongo's own session, once created, has the routing baked in immutably).
  - **Superseded twice over.** `20260810000038` re-declared `prevent_booking_tampering()` to guard `credit_amount_applied`, stating it reproduced "every clause from `20260810000009` verbatim" — which it did, silently copying back the three clauses 011 had just removed. That went unnoticed because the guard reverts values instead of raising, so the write kept returning success while persisting nothing. `20260810000056` resolves it in the other direction: the guard stays (011's "never read back" premise expired once `adminPayments.ts` began reconciling the pair against `price_amount` and `venueEarnings.ts` began summing `venue_amount`), and the write moves to the service_role-only `set_booking_marketplace_split()`. Confirmed on staging in both directions by `scripts/verify-staging-marketplace-split-snapshot.ts`.

## Phase 2 — Venue onboarding (live-tested)

`lib/services/paymongoAccounts.ts`: `createPayMongoMerchantAccount()` (`POST /v2/accounts`, confirmed live — flat `{"type":"merchant"}` body, *not* nested under `data.attributes`), `createIdentityVerificationSession()` (PayMongo's own hosted KYC page — AIR/Rally never collects or stores identity/bank documents), `activatePayMongoAccount()` (fire-and-forget; its response is never trusted, see above). `lib/actions/venueOnboarding.ts#startPaymongoOnboardingAction` is idempotent — reuses an already-linked account rather than creating a second one on a repeat click, mirroring a gap the earlier (paused) Stripe Connect design caught. `PaymongoOnboardingCard` on the venue management page shows the 4 states.

## Phase 3/4 — Checkout extension + commission math

`lib/services/commission.ts#calculateMarketplaceSplit()`: integer-only (`Math.round()` once for the 5% fee, the venue's 95% is always the *remainder by subtraction* — never separately rounded — so the two are guaranteed to sum exactly to the gross amount for every input, a property of the arithmetic itself). `lib/actions/checkout.ts` computes the split fresh from the booking's own `price_amount` and only attaches it when `getCourtDisplayInfo()` reports the venue's `paymongo_activation_status === "activated"` — every other status (including a venue that's never started onboarding) transparently falls back to the pre-existing, non-split checkout.

`createPayMongoCheckoutSession()` now calls `POST /v2/checkout_sessions` for every session (confirmed superset of v1 for creation — same line-item/metadata/URL fields — per PayMongo's own OpenAPI schema), attaching `split_payment: { recipients: [{ merchant_id: <AIR/Rally's own PayMongo org id>, split_type: "fixed", value: platformFeeAmount }], transfer_to: <venue's org id> }` only when a split applies. `split_type: "fixed"` is used deliberately over `"percentage_net"` (undocumented rounding behavior) — both amounts are computed by AIR/Rally itself, so PayMongo never needs to round anything.

**New required env var**: `PAYMONGO_PLATFORM_ACCOUNT_ID` — AIR/Rally's own PayMongo Platforms account id, needed as the `recipients[].merchant_id`. There is no API to discover this (this account's key can't call `/v1/organizations`); it must come from the PayMongo Dashboard. Only required once a venue actually reaches `activated`.

**Deliberately not enabled yet: `pass_on_fees`.** It's a real, confirmed field on the same endpoint (documented as "Now supports automatic pass on fees!"), but whether PayMongo reports the *paid* Payment's `amount` as the original total or as that total plus the passed-on fee is unconfirmed — and `confirm_paymongo_booking_payment()`'s amount-integrity check compares against the booking's own `price_amount`. Guessing wrong would silently break confirmation for every split payment. Deferred to live TEST MODE verification rather than assumed, per this project's explicit "do not guess API behavior" rule.

## Phase 5 — Webhook authority for split payments

No new payment-confirmation code was needed: `confirm_paymongo_booking_payment()`'s amount check compares against the booking's own `price_amount`, which a split payment (with `pass_on_fees` still disabled — see Phase 3/4 above) never differs from, so the exact same webhook handler, the exact same idempotent `WHERE status = 'pending'` guard, and the exact same "redirect arrives before webhook" fallback (`reconcilePaymongoPendingBooking()`, which re-verifies against a real PayMongo API call rather than ever trusting the browser's mere presence at the confirmation URL) already cover split bookings correctly, with no split-awareness required at that layer.

**A real, honestly-reported gap**: unlike Stripe Connect's `account.application.deauthorized`, no PayMongo webhook event for "a child account was disabled/revoked *after* activation" was found anywhere in the documentation I could access (searched the full onboarding-webhooks catalog, the general webhook-events catalog, and the Linked Accounts guide — nothing beyond `merchant.activated`/`merchant.declined`, which only fire once, at initial review). The `Relationship` resource does carry its own `enabled` flag, separate from `activation_status`, but no webhook event for a change to it is documented. This means `venues.paymongo_activation_status` could theoretically go stale if PayMongo disabled an account through some administrative action with no corresponding webhook.

The financial risk from this gap is bounded, not open-ended: `paymongo_activation_status` only gates whether a **new** checkout session gets a `split_payment` attached (Phase 3/4's gate in `lib/actions/checkout.ts`). If PayMongo actually rejects a split payment attempt against a disabled account, the customer's payment simply fails at PayMongo's end — no `checkout_session.payment.paid` webhook ever fires, and the booking stays `pending` (the same, already-documented "abandoned pending booking" degradation this codebase already accepts for ordinary payment abandonment). No scenario was found where this gap could cause funds to be misrouted or a booking to be wrongly confirmed — only where a stale "activated" status could cause a customer's payment attempt to fail late, which is a UX gap worth flagging to PayMongo support, not a correctness bug worth guessing a fix for.

## Phase 6 — Refunds: blocked, not implemented

No documentation anywhere found (Refunds, QRPH Refunds, Refund errors, the Create Refund reference, or the Linked Transactions resource table) describes how a refund interacts with a split payment — whether both legs reverse atomically, whether the platform fee is actually non-refundable at the API level (vs. a business-policy statement), or what happens once the venue's share has settled. A live empirical test wasn't possible either: this project's earlier research already hit `available_balance_insufficient` on a plain, non-split ₱1.00 refund, since TEST MODE funds aren't refund-eligible until PayMongo's own settlement window passes (~3 days). No refund code was written. Needs either written PayMongo confirmation or a real test against an already-settled split payment.

## Phase 7 — Live TEST MODE verification

**293/293 tests passing** (34 suites), clean `tsc --noEmit`, clean `eslint` (same 4 pre-existing React Compiler warnings, unrelated to this work), clean `next build`.

**Real, live proof of the `split_payment` mechanism itself** (direct PayMongo API + real browser-completed payment, independent of AIR/Rally's own booking flow):

1. Created a real Checkout Session with `split_payment.recipients: [{ merchant_id: <our real platform org id>, split_type: "fixed", value: 2500 }], transfer_to: "org_test_merchant"` (a TEST MODE mock child account). PayMongo accepted session creation (`200`), but the **hosted checkout page itself displayed a real error at payment time**: *"No such merchant with id org_test_merchant."* This is a genuine, informative finding: `split_payment` validates both sides against real accounts only at actual payment submission, not at session creation — and confirms TEST MODE's mock account-creation endpoint (`POST /v2/accounts`) always returns the same placeholder id (`org_test_merchant`), which is not a real, transactable linked account. `GET /v2/relationships` independently confirmed this — it returned `{"data":[],"has_more":false}` even after creating that mock account, meaning no real parent-child relationship was ever established by the mock flow.
2. To still exercise the real mechanism, created a second, degenerate-but-fully-real session: `recipients: [{ merchant_id: <our real platform org id>, split_type: "fixed", value: 2500 }], transfer_to: <our real platform org id>` (both sides real, same account). Completed a real payment via the hosted page (PayMongo's official TEST MODE card `4343434343434345`) — succeeded, `pay_1m1z5oZXv9cUMyAr3DcmXKoh`. Retrieving the resulting Payment confirmed a real `split_payment` sub-object: `{"status":"succeeded","split_payments":[{"recipient_organization_id":"<our real org id>","amount":50000,"fee":3250,"foreign_fee":500,"net_amount":46250,...}]}` — proving `split_payment` is a genuine, functioning mechanism that PayMongo actually executes, not silently ignored.
3. **What this does NOT prove**: the actual two-party 95%/5% amount distribution (`platformFeeAmount`/`venueAmount` landing in two different real accounts), since the degenerate test necessarily had only one effective destination. Proving that specifically requires a second real, fully-onboarded child account — which needs either a genuinely completed onboarding flow (blocked: TEST MODE's identity-verification step returns a non-functional `https://example.com/test-verification` placeholder URL, not a completable hosted flow) or PayMongo support provisioning a real TEST MODE Platforms sandbox relationship.
4. **Also confirmed**: `pass_on_fees: false` was correctly echoed back on the checkout session (confirming the field is real and respected, not silently dropped) — consistent with this integration deliberately not enabling it. The payment-level `fee`/`foreign_fee`/`net_amount` (3250/500/46250 on a ₱500 charge) exactly matched the earlier non-split TEST MODE research's own observed numbers, confirming these are PayMongo's ordinary card-processing fee mechanics, unrelated to and unaffected by `split_payment`.

**Not yet run**: a full, real, own-app end-to-end pass (real venue row → real booking → real webhook → real `platform_fee_amount`/`venue_amount` persisted and verified via read-only query). This needs a real venue-owner test session, which this project deliberately avoided creating through browser automation with a stored password (see the session's own credential-handling discussion) — the underlying `lib/actions/venueOnboarding.ts` and `lib/actions/checkout.ts` wiring is fully covered by mocked tests instead.

## Phase 8 — Production preparation: agreement, readiness, hardening, refunds, admin/owner payment visibility, launch gates

Built with PayMongo marketplace payments still explicitly disabled in production (`PAYMONGO_MARKETPLACE_SPLIT_ENABLED`/`PAYMONGO_REFUND_EXECUTION_ENABLED` both default `false` — see below) — this phase is the application/product layer that doesn't depend on the still-unresolved two-account PayMongo verification, per the task's own explicit scope boundary. Nothing here weakens or touches the existing Stripe flow.

**User agreement acceptance.** `agreement_acceptances` (new table, additive, RLS select-only for self/admin) plus `record_agreement_acceptance()` (`SECURITY DEFINER`, granted to `anon, authenticated` because the "email confirmation pending" signup path has no session yet). Called from `lib/actions/auth.ts` immediately after a real `supabase.auth.signUp()` succeeds, using that response's real user id — never a client-supplied boolean. `lib/legal.ts#CURRENT_AGREEMENT_VERSION` is the single source of truth for "current"; `signup/page.tsx` gates the submit button on a checkbox validated server-side too (`signUpSchema`'s `agreedToTerms` refine). Because the RPC is necessarily anon-callable, it independently verifies `auth.uid() = p_user_id` whenever a session *does* exist, closing the "forge another user's acceptance" hole a first draft of this migration left open — caught and fixed during this phase's own financial/database safety audit before the migration was ever applied.

**Venue readiness.** `lib/services/venueReadiness.ts#getVenueReadiness()` returns a structured `VenueReadiness` (per-item `complete`/`action_required`/`pending_verification`/`not_applicable`, never one boolean) covering business info, location, courts, pricing, operating hours, platform approval, and — only when `ACTIVE_PAYMENT_PROVIDER=paymongo` — PayMongo onboarding. `VenueReadinessChecklist` displays it read-only on the venue management page. A real product gap was closed alongside this: there was no owner-facing UI for `venue_operating_hours` at all despite correct RLS existing since Phase 4A, so a minimal one-window-per-day editor (`VenueOperatingHoursEditor`, `setOperatingHoursAction`) was built since the checklist needs to be actually actionable.

**Payment UX/reconciliation hardening.** Reviewed and hardened the existing checkout/webhook/reconciliation paths (duplicate submission, abandoned checkout, idempotency) — the underlying mechanisms (webhook-authoritative confirmation, the `WHERE status = 'pending'` idempotency guard, the "redirect arrives before webhook" fallback) already existed from Phase 4B/PayMongo TEST MODE and needed hardening/tests, not a redesign.

**Refund architecture** (`lib/services/refunds.ts`, `lib/actions/refund.ts`, `booking_refunds` table). Deliberately a full second, independent Stripe client (`getStripeClient()` duplicated, never importing `lib/services/payments.ts`) — the provider-isolation-via-duplication posture this codebase uses everywhere. Audit-first: a `pending` `booking_refunds` row is written *before* the provider is ever called, then updated to `succeeded`/`failed`. The refundable amount is always recomputed fresh from the sum of real `succeeded` rows (never a separate counter), so a retried/duplicate request naturally self-limits without a separate idempotency key. No platform/venue split-reversal columns exist — that accounting is unproven for PayMongo, and this table deliberately doesn't guess it. PayMongo refund *execution* is hard-gated behind `PAYMONGO_REFUND_EXECUTION_ENABLED`; Stripe refunds are real, unsplit, unautomated-elsewhere API calls. Admin-only end to end: `refundBookingAction` and every `booking_refunds` write require `requireAdmin()` (see below) in addition to RLS's own `is_admin()` check.

**`lib/services/admin.ts#requireAdmin()`** — a single shared admin-gate check (session + `profiles.role === 'admin'`), extracted once a second admin-only feature (payment monitoring) needed the exact same check `refund.ts` already had inline, so the rule can't drift between call sites. RLS remains the real enforcement; this only produces a clean error before ever attempting a privileged call.

**Admin payment monitoring** (`/admin/payments`, `lib/services/adminPayments.ts`). Relies entirely on the existing `bookings`/`booking_refunds`/`venues` SELECT RLS (all already include `is_admin()`) — no service-role key anywhere in this codebase. Derives a `PaymentLifecycleStatus` (`pending`/`confirmed`/`cancelled`/`partially_refunded`/`refunded`) and a list of `reconciliationFlags` per booking, every flag grounded in real stored columns only — never a guess about provider behavior: a confirmed booking missing `paid_at` or a provider payment id, a `platform_fee_amount`/`venue_amount` pair that doesn't sum to `price_amount`, a refund total exceeding the price, a failed refund attempt with no later success, or a `pending` booking older than 24h (likely abandoned checkout). Separately surfaces venue-level issues: `pending_review`/`suspended` venues, and PayMongo venues stuck mid-onboarding or declined. Shows provider payment/refund ids (safe, already-used identifiers) — never a secret key.

**Venue earnings visibility** (added to `/list-your-court/[venueId]`, `lib/services/venueEarnings.ts`, `VenueEarningsCard`). Shows what customers have paid for a venue's bookings, refunded amounts, and — only when actually snapshotted (PayMongo marketplace split) — the requested venue-side split. Carries a prominent, explicit banner and consistently avoids the words "received"/"paid out"/"in your account" anywhere: **Stripe Connect payouts were never implemented** (every Stripe booking's full amount settles to AIR/Rally's own Stripe account, not the venue's — see Phase 4B.5, still paused), and PayMongo settlement/payout mechanics for Child Merchants remain unverified. A booking showing "Paid" means the customer's payment succeeded, nothing about the venue's own funds.

**Launch gates** (`lib/paymongoLaunchGates.ts`). `PAYMONGO_MARKETPLACE_SPLIT_ENABLED` and `PAYMONGO_REFUND_EXECUTION_ENABLED`, both independent, both default `false` (strict `=== "true"` string comparison), checked as an *additional* AND-condition alongside (never instead of) real per-venue/per-row DB state — so no single env mistake or DB state change can alone activate unverified PayMongo marketplace behavior. `ACTIVE_PAYMENT_PROVIDER` still independently defaults to `stripe`.

**Financial/database safety audit.** Reviewed rather than duplicated: confirmed RLS-only enforcement everywhere (no service-role key exists anywhere in this codebase), confirmed no Zod validation schema anywhere accepts a client-supplied monetary amount (the only `amount` parameter in the entire actions layer is the admin-only, server-capped `refundBookingAction`), and confirmed every new SECURITY DEFINER function's bypass/ownership logic. One real gap found and fixed pre-application: `record_agreement_acceptance()`'s first draft trusted `p_user_id` from any anon caller with no ownership check at all — fixed to verify `auth.uid() = p_user_id` whenever a session exists (see above).

**Migrations drafted, not yet applied**: `20260810000012_agreement_acceptances.sql`, `20260810000013_booking_refunds.sql` — both purely additive, presented for explicit approval before running against the live database, same process as every migration in this project.

## Owner Experience — visual venue dashboard, image upload, venue delete

Explicitly scoped away from PayMongo/payments — pure marketplace-presentation work on top of infrastructure (the `venue-images` Storage bucket, the `court_images` table) that already existed from Phase 3, unused until now.

**Card-based owner dashboard.** `OwnerVenueGrid`/`OwnerVenueCard` replace the old text-row `OwnerVenueList` on `/list-your-court`: cover photo (or a placeholder icon when none), name, city, a real court count, and a status badge. `listVenuesByOwnerWithSummary()` (`lib/services/venues.ts`) fetches this in 3 flat queries regardless of how many venues the owner has — venues, then a batched court-count and a batched venue-level-cover-image query (`court_images.court_id is null`, first-per-venue reduced in JS) — no new RPC.

**Image upload — client-direct to Storage, not a Server Action.** `ImageUploadManager` (shared between venue-level and court-level photos) uploads straight from the browser to Supabase Storage using the ordinary browser client, then inserts the `court_images` row — both steps already fully covered by existing owner-scoped RLS. Routing binary uploads through a Server Action would have needed raising Next's default 1MB body limit project-wide for no security benefit, since RLS (not app code) is the real boundary either way. Deletion (`deleteImageAction`) stays a Server Action — no bandwidth concern, and it needs to remove the DB row and the Storage object as one coordinated operation (row first, then object — an orphaned Storage object is harmless; the reverse risks a dangling row pointing at a 404). A new migration (`20260810000018_venue_images_bucket_limits.sql`) added bucket-level `file_size_limit`/`allowed_mime_types` (5MB, JPEG/PNG/WEBP) as defense-in-depth, since RLS has no concept of file size or type.

**A real, pre-existing production bug found and fixed.** Live staging verification of this feature's upload path (the first time this project ever exercised it — confirmed via `grep` that zero `.upload()` calls existed anywhere in `src/` before this phase) caught that the `venue-images` bucket's owner-scoped `storage.objects` RLS policies (from Phase 3's `20260809000009_venue_images_storage.sql`) had silently never worked: `(storage.foldername(name))[1]` inside a correlated subquery against `public.venues v` resolved the bare `name` to `v.name` (the venue's own display name) instead of the intended `storage.objects.name` (the uploaded object's path), due to ordinary SQL scoping — an inner-scope column shadows an outer one of the same name. Every real owner's insert/update/delete against this bucket had always been silently rejected; only the SELECT policy (which never referenced `name`) worked, so nothing caught this until a write was actually attempted. Fixed in `20260810000019_fix_venue_images_storage_rls.sql` by qualifying the reference as `objects.name`. **Applied to staging only** — this bug is still live in production as of this writing, since production has never had an upload path to exercise it either; fixing it there needs the same explicit-approval migration process as any other production change.

**Venue delete — reuses an already-existing RLS policy, no new migration.** `deleteVenueAction` → `deleteVenue()` is a plain `DELETE`; the existing policy (`owner_id = auth.uid() AND status = 'draft'`, or admin — from Phase 2) is the entire enforcement. The FK chain (`venues` →cascade→ `courts` →**restrict**→ `bookings`) means this can only ever succeed for a venue with zero booking history, which a still-draft venue always has (booking creation requires an active, listed venue) — so draft-only deletion can never silently destroy booking/payment records; a venue with real history simply can't reach `DELETE` in the first place via any path, cascade or otherwise. The UI only shows "Delete draft" on draft-status cards — a UX nicety mirroring the RLS boundary, not the boundary itself, same posture as every other status-gated control in this codebase.

**Out of scope, deliberately:** the public marketplace grid (`CourtCard.tsx`, `venue_marketplace` view) — this phase is the owner dashboard only, no cover-image column was added to the public view; image reordering; bulk delete; deleting a non-draft venue (no self-service archive/suspend path exists for owners today — only admins can change a venue's status past `draft`/`pending_review`, so there's no safe "soft delete" to build without separate scope).
